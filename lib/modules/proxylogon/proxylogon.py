        ...
        async with self.semaphore:
            ...
            try:
                async with session.request(
                    ...
                    _tmp_headers = ujson_dumps(
                        _default_record["data"]["http"]["result"]["response"]["headers"]
                    )
                ...
                if not exchange_version:
                    ...
                    url_step2 = (
                        f"{target.url[:-1]}/owa/auth/logon.aspx?"
                    ...
                    async with session.request(
                        ...
                        if target.method in ["GET", "POST", "PUT", "DELETE", "UPDATE"]:
                            ...
                            try:
                                ...
                                exchange_version = findall(
                                    '<link\s[^>]*href="/owa/[^"]*?([\d.]+)/themes/resources/favicon.ico',
                                    response_test,
                                )
                                if exchange_version:
                ...
                if exchange_version:
                    ...
                    if len(part_list) != 4:
                        url_step3 = (
                            f"{target.url[:-1]}/ecp/Current/exporttool/"
                ...
                if exchange_version:
                    ...
                    result = create_error_template(
                        target, error_str="", status_string="success-not-contain"
                    )
                if result:
                    if not result["ip"]:
