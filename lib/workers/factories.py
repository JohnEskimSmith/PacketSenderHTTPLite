...
from lib.util import encode_files_payload, is_ip, is_network

RESERVED_CHAR = ";"  # for spliting endpoints, I don't want use narg


def create_target_http_protocol(
    raw_str: str, target_config: TargetConfig, target_type: str
) -> Iterator[Target]:
    """
    ...
    if target_type == "network":
            ...
            if target_config.list_payloads:
                for payload in target_config.list_payloads:
                    ...
                    if (target_config.single_payload_type).lower() == "data":
                        ...
                        target_config.single_payload_type
        ...
        if target_config.list_payloads:
            for payload in target_config.list_payloads:
                ...
                target_instance = Target(
                    payload=payload, additions=additions, **_struct, **kwargs
                )
                yield target_instance
        elif target_config.python_payloads:
            payloads_generator = get_generator(target_config)
...
def create_targets_http_protocol(
    raw_str: str, target_settings: TargetConfig
    ...
    if raw_targets:
        for raw_target in raw_targets:
            for target in create_target_http_protocol(
                str(raw_target), target_settings, target_type
            ):
                yield target
