...
def parse_sqs_message_yandex_msgpack(event: Dict) -> Optional[Tuple[Optional[Dict], str, str]]:
    try:
        body_current_message = access_dot_path(event['messages'][0], 'details.message.body')
...
def create_args_list(settings: Dict,
                     files: Tuple[str, str]) -> List[str]:
...
def parse_args_from_sqs_message(event: Dict) -> Optional[List[str]]:
    struct_from_message: Optional[Tuple[Optional[Dict], str, str]] = parse_sqs_message_yandex_msgpack(event)
